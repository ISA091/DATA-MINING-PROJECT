...
# Each dataset contains 5.000 samples (stratified) from the original dataset of Tuesday's traffic
...
for C_range in range(1,10,1):
    for gamma_range in range (1,10,1):
        ...
        f1_scores = []
        for i in range(5):
            clf = SVC(C=C_range, kernel='rbf', gamma=gamma_range)
        ...
        print("C =" ,C_range, "Gamma =",gamma_range)               
        ...
        ax1.scatter(C_range,gamma_range,np.mean(accuracies)) 
        ax2.scatter(C_range,gamma_range,np.mean(precisions)) 
        ax3.scatter(C_range,gamma_range,np.mean(recalls)) 
        ax4.scatter(C_range,gamma_range,np.mean(f1_scores)) 
...
for C_range in range(1,3,1):
    for gamma_range in range (1,3,1):
        accuracies = []
        recalls = []
        precisions = []
        f1_scores = []
        for i in range(5):
            gamma_range=gamma_range/100
            C_range=C_range/100
            clf = SVC(C=C_range, kernel='rbf', gamma=gamma_range)
            clf.fit(X_train, y_train)
            y_pred = clf.predict(X_test)
...
for C_range in range(1000,3000,1000):
    for gamma_range in range (1000,3000,1000):
        accuracies = []
        recalls = []
        precisions = []
        f1_scores = []
