...
# Each dataset contains 5.000 samples (stratified) from the original dataset of Tuseday's traffic
...
# We are going to run the classifier 10 times and append average results to the arrays
for i in range(10):
    clf = SVC(C=10, kernel='rbf', gamma='scale')
    start_time = time.time()
    clf.fit(X_train, y_train)
...
# Now we advance to the file Gamma_AND_C.py to select the best pair of values
# of C and gamma